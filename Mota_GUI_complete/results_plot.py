...
#============================================================================
#  Save_File 儲存檔案  V2.0    by Hung1    2020/07/18
#----------------------------------------------------------------------------
#  可以記錄每一回的訓練結果，並保存至檔案。
#  V1.0    2020.04.27
#  V1.1    2020.05.10
...
class Save_File:
    ...
    def save_file(self):
        ...
        np.save(file_name, self.get_data(data_upper_limit=-1))
   ...
   # unicode_font = 'DFKai\-SB'        # unicode字體
    ...
    def get_figure(self,data, color, avglevel=-1, exclude_zero=False, **kwargs):
        """產生位圖"""
        fig = plt.Figure(**kwargs)
        ax = fig.add_subplot()
        #fig, ax = plt.subplots(**kwargs)  # 會在Spyder的Console產生圖片
        if self.check_contain_chinese(data['env_name']):
            ax.set_title(data['env_name'], fontproperties='DFKai\-SB', fontsize=16, color='b')
        else:
        ...
        if exclude_zero:
            x_data = []
            y_data = []
            for x, y in enumerate(data['score'], 1):
                if y != 0:
                    x_data.append(x)
                    y_data.append(y)
        else:
            x_data = range(1, len(data['score'])+1)
            y_data = data['score']
        data_length = len(y_data)
        # 刻度定位器設置
        # 參考：https://matplotlib.org/3.1.0/api/ticker_api.html
        if not y_data or max(y_data) - min(y_data) <= 5:
            ax.yaxis.set_major_locator(plt.MultipleLocator(1))
        if data_length <= 5:
            ax.xaxis.set_major_locator(plt.MultipleLocator(1))
        # 範圍線繪製
        ax.plot(x_data, y_data, label=data['algorithm'], alpha=0.2, color=color)
        # 平均線繪製
        y_avg = []
        if y_data:
            if avglevel < 0:
...
if __name__ == '__main__':
    ...
    root = tk.Tk()
    sf = Save_File('可以中文標題environment', 'algorithm')
    startTime = time.perf_counter()
    for score in np.sin(np.pi*np.arange(0, 90, 0.9)):
        sf.sampling(score)
    print('   sampling time:', time.perf_counter() - startTime)
    startTime = time.perf_counter()
    data = sf.get_data()
    fig = sf.get_figure(data, color='coral', #color='steelblue'/'coral'
