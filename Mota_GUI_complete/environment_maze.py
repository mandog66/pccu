...
class Mota_Maze():
    ...
    def step(self, action: int, return_state: bool = True,
        ...
        # 計算獎勵值
        if ending in Mota_Maze.ENDDING_REWARD:
            reward = Mota_Maze.ENDDING_REWARD[ending]
        else:
            # 行動後的主角狀態
            after_state = self.get_player_state()
            reward = np.sum((after_state - before_state) * Mota_Maze.REWARD_RATE)
        # 無意義行動獎勵值
        if reward == 0:
            reward = meaningless_actions_reward
        # 限制行動輸入
        if ending != 'invalid' and ending != 'continue':
