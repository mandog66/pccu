...
#  DoubleDeepQNetwork
#----------------------------------------------------------------------------
#  使用Mota_Maze環境學習  Ver.1.1  by Yaowang  2020.05.09
#============================================================================
#============================================================================
# 參考
...
#============================================================================        
    def learn(self):
        ...
        # Q_taget中的Q_eval的Q_value      
        Q_eval_next = self.eval_model.predict(minibatch[:,-self.features:]) 
               
        # 在相對應的動作進行反傳遞
        Q_target = Q_eval.copy()
        batch_index = np.arange(self.batch_size, dtype=np.int32)
        # eval_估計動作
        Q_max_eval_next = np.argmax(Q_eval_next, axis=1)       
        
        eval_action_index = minibatch[:, self.features].astype(int)       
        reward = minibatch[:, self.features+ 1]
        
        # DDQN
        # Q_target = R+ γQ (Q_next,argmax_Q(Q_next))     
        Q_target[batch_index, eval_action_index] = reward + self.gamma * Q_next[batch_index, Q_max_eval_next.astype(int)]
        # target_更新
